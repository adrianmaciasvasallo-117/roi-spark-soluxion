...
const Metric = ({ label, value, sub, color, bg = 'white', borderColor, size = 'sm' }: MetricProps) => {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: bg, borderLeft: borderColor ? `3px solid ${borderColor}` : undefined }}
    >
...
const SectionLabel = ({ children }: { children: React.ReactNode }) => (
  <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: LABEL }}>{children}</p>
);
...
const ResultadosSection = ({ results, data }: Props) => {
  ...
  const costChartData = {
    ...
    datasets: [{ data: [results.costoActual, results.costoTrasAutomatizacion], backgroundColor: [LOSS, PREMIUM], borderRadius: 6 }],
  };
  ...
  const savingsChartData = {
    labels: months,
    datasets: [{ label: 'Ahorro acumulado (€)', data: cumSavings, borderColor: POSITIVE, backgroundColor: 'rgba(46,158,111,0.12)', fill: true, tension: 0.3, pointRadius: 3 }],
  };

  const roiChartData = {
    labels: ['ROI (%)'],
    datasets: [{ data: [Math.max(0, results.roi)], backgroundColor: [POSITIVE], borderRadius: 6 }],
  };

  const selectedItems = data.selectedAutomations.map((id) => AUTOMATIONS_CATALOG.find((a) => a.id === id)!).filter(Boolean);
  const blueShades = ['rgba(30,58,138,0.9)', 'rgba(30,58,138,0.7)', 'rgba(30,58,138,0.5)', 'rgba(30,58,138,0.35)', 'rgba(30,58,138,0.2)'];
  const breakdownChartData = selectedItems.length > 0
    ? { labels: selectedItems.map((a) => a.nombre), datasets: [{ data: selectedItems.map((a) => results.allocationPrices[a.id] || 0), backgroundColor: blueShades.slice(0, selectedItems.length) }] }
    : null;

  return (
    <div className="space-y-6">
      {/* A2 FIX: Coste actual — now shows costoTiempo instead of duplicate costoActual */}
      <div>
      ...
      <div className="flex justify-center">
        <div className="rounded-lg border p-6 shadow-sm text-center w-full max-w-md" style={{ backgroundColor: results.ahorroMensualNeto > 0 ? POSITIVE_BG : 'white' }}>
