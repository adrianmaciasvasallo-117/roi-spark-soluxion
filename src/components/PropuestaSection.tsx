...
const PropuestaSection = ({ data, results, onReset, onUpdateData }: Props) => {
  ...
  const validate = (): string | null => {
    ...
    if (!data.nombreCliente?.trim()) return 'El campo "Nombre del cliente" es obligatorio.';
    if (!data.emailCliente?.trim() || !data.emailCliente.includes('@')) return 'Introduce un email válido.';
    if (!data.rgpdChecked) return 'Debes aceptar el tratamiento de datos (RGPD).';
    return null;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!blob || blob.size === 0) throw new Error('El PDF generado está vacío.');
      setPdfBlob(blob);
      setState('pdf_ready');
  ...
  const handleSend = async () => {
    ...
    if (!pdfBlob || pdfBlob.size === 0) {
      toast({ title: 'PDF necesario', description: 'Primero genera la propuesta (PDF).', variant: 'destructive' });
      return;
    }
    if (!data.emailCliente?.trim() || !data.emailCliente.includes('@')) {
      toast({ title: 'Email inválido', description: 'Introduce un email válido antes de enviar.', variant: 'destructive' });
      return;
    }
    if (!data.rgpdChecked) {
      toast({ title: 'RGPD', description: 'Debes aceptar el tratamiento de datos.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      if (uploadError) throw new Error('No se pudo subir el PDF. Intenta de nuevo.');
      ...
      if (error) throw new Error(typeof error === 'object' && 'message' in error ? (error as any).message : 'Error al invocar la función de envío.');
      if (responseData && responseData.error) throw new Error(responseData.error);

      setState('sent');
      setSuccessDialogOpen(true);
      toast({ title: '¡Email enviado!', description: `Propuesta enviada a ${recipient}` });
    } catch (e: any) {
      setState('error');
      setErrorDetail(e?.message || 'Error desconocido al enviar el email.');
      toast({ title: 'No se pudo enviar el email', description: 'Revisa el email del cliente y tu conexión.', variant: 'destructive' });
    } finally {
      ...
      <>
        <Card className="text-center py-12">
          <CardContent className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">Se ha enviado la propuesta a {data.emailCliente}</p>
        ...
        <Dialog open={successDialogOpen} onOpenChange={setSuccessDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>La propuesta ha sido enviada a: <strong>{data.emailCliente}</strong></DialogDescription>
            </DialogHeader>
          </DialogContent>
        </Dialog>
      </>
  ...
  return (
    <div className="space-y-5">
      {/* A5: Email config fields moved here */}
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Configuración del envío</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex items-start gap-2">
            ...
            <Label htmlFor="rgpd" className="text-xs leading-relaxed">
              Acepto el tratamiento de datos para recibir la propuesta por email.
            </Label>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-3 flex-wrap">
            ...
            <Button onClick={handleSend} disabled={!canSend || isSending} variant="secondary">
          ...
          {state === 'error' && (
            <div className="rounded-md border border-destructive/30 bg-destructive/5 p-4 space-y-2">
              <p className="text-sm font-medium text-destructive">No se pudo enviar el email. Revisa el email del cliente y tu conexión.</p>
              {errorDetail && (
                <details className="text-xs">
                  <summary className="cursor-pointer text-muted-foreground">Detalles técnicos</summary>
                  <pre className="mt-1 whitespace-pre-wrap break-all bg-muted p-2 rounded">{errorDetail}</pre>
                </details>
              )}
              {pdfBlob && (
                <Button variant="outline" size="sm" onClick={() => triggerDownload(pdfBlob)}>
              ...
              <Button variant="outline" size="sm" onClick={() => setState('pdf_ready')}>
                Reintentar envío
              </Button>
            </div>
          )}
